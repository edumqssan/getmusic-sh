...
# Function that obtains data with youtube-dl and saves it to ~/Music
get() {
	# Get directories (aka. dir)
	local current_dir=$(pwd)
	local target_dir=$(echo -n ~/Music)
	
	# Get music title, used for the download messages
	local title=$(/usr/bin/env youtube-dl -s -e $1)

	if [[ "$title" ]]; then
		# Change directory to the target directory (~/Music)
		cd $target_dir
		
		# Check verbose flag state value and define youtube-dl arguments
		if [ "${verbose}" -eq 2 ]; then
			local cmd_argv="-x -c --audio-format mp3 --embed-thumbnail -o %(title)s.%(ext)s"
		else
			local cmd_argv="-q -x -c --audio-format mp3 --embed-thumbnail -o %(title)s.%(ext)s"
		fi
